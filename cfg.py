...
class derivations():
	def __init__(self, cfg_in, parse):
		self.input_string = parse
		self.max_depth = 5
		self.store = []
		self.cfg = cfg_in
		self.is_match = False
		self.traverse(self.cfg.rules["S0"], 1)

		if not self.is_match:
			print("string", self.input_string, "is not generated by this CFG")



	def traverse(self, rule, current_depth):
		if current_depth <= self.max_depth:
			for production in rule:
				if len(production) > 1:
					# Traverse left
					x = self.traverse(self.cfg.rules[production[0]], current_depth+1)
					# Traverse right
					y = self.traverse(self.cfg.rules[production[1]], current_depth+1)

					if (x != None) and (y != None): # If neither of the branches exceed the depth limit
						if type(x) == tuple:
							x = "".join(x)			# Join the returned list of x
						if type(y) == tuple:
							y = "".join(y)			# Join the returned list of y

						if x+y == self.input_string: # If the input string matches the concatenation of the two x,y nodes, return "MATCH"
							print(self.input_string, "MATCH")
							self.is_match = True
						return(x+y) # Return the x+y
				else:
					return production 	# Return the terminal (leaf node)
		else:
			return None # Depth has been exceeded
